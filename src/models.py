...
class MEI(torch.nn.Module):
    ...
    def compute_score(self, h: torch.Tensor, r: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.n_w and self.config.norm != 'bn':
            ...
            M = self.norm_w(M)
        M = self.dropout_w(M)
        M = M.view(self.config.Cr, -1)  # (cr, ce*ce)
        ...
        r = r.view(-1, self.config.Cr)  # (batch*k, cr)

        Mr = torch.matmul(r, M)  # (batch*k, ce*ce)
        if self.config.n_mr:
    ...
    def compute_loss_total(self, score: torch.Tensor, y: torch.Tensor, active_e: torch.Tensor):
        ...
        NOTE: using self.h, self.r... reference to the cached tensors in previous self.forward() call
        """
